document.addEventListener('DOMContentLoaded', () => {
    ...
    d3.csv('MSPUS_processed.csv').then(data => {
        ...
        const sceneDescriptions = [
            'This scene highlights the overall steady increase in housing prices over the past 60 years, with a significant rise in the 2020s.',
            'As can be seen, some of the most major changes in housing prices occurs during the 2008 financial crisis, showing the drop in housing prices and subsequent recovery.',
            'As of recently, this graph highlights the significant increase in housing prices since the COVID-19 pandemic started in 2020.'
        ];

        function addAnnotations(annotations, keepExisting = false) {
            if (!keepExisting) {
        ...
        document.getElementById('scene2-btn').addEventListener('click', () => {
            addAnnotations(annotationsScene2, true);
